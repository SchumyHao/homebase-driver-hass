...
var hbinfo = {
  ...
  get_actions: state => {
    ...
    var operation_list = state.attributes.hvac_modes;
    if (operation_list) {
      var mode_list = [];
      if (operation_list.indexOf('auto') >= 0)
        mode_list.push('auto');
      if (operation_list.indexOf('heat_cool') >= 0) {
        mode_list.push('cool');
        mode_list.push('heat');
      } else {
    ...
    if (supported_features & SUPPORT_TARGET_TEMPERATURE_RANGE) {
      ...
      if (temp)
        actions.temperature = ['up', 'down', 'num', 'max', 'min'];
    } else if (supported_features & SUPPORT_TARGET_TEMPERATURE) {
      var temp = state.attributes.temperature;
      if (temp)
        actions.temperature = ['up', 'down', 'num'];
    }
  ...
  get_states: state => {
    ...
    var set_mode = state.state
    if (set_mode === 'auto')
      states.mode = 'auto';
    else if (set_mode === 'cool')
      states.mode = 'cool';
    else if (set_mode === 'heat')
      states.mode = 'heat';
    else if (set_mode === 'dry')
      states.mode = 'dry';
    else if (set_mode === 'fan_only')
      states.mode = 'fan';
    else if (set_mode === 'eco')
      states.mode = 'energy';
    else if (set_mode === 'off')
      states.mode = 'off';
    else if (set_mode === 'idle')
      states.mode = 'idle';
