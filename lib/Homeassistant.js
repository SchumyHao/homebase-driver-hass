...
function Homeassistant(userAuth) {
  this.authType = userAuth.ext3 === TYPE_OAUTH2 ? TYPE_OAUTH2 : TYPE_PASSOWRD
...
Homeassistant.prototype._request = function (method, path, body, options) {
  return new Promise((resolve, reject) => {
    ...
    if (this.type === TYPE_PASSOWRD) {
      options.headers['x-ha-access'] = this.password;
    } else if (this.type === TYPE_OAUTH2) {
...
Homeassistant.prototype.authRefreshToken = function() {
  ...
  const body = 'grant_type=refresh_token&refresh_token=' + userAuth.refreshToken + '&client_id=' + encodeURIComponent(userAuth.ext2)
